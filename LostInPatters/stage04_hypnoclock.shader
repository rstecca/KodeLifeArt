...
void main()
{
    ...
    coords = vec2(coords.x/resolution.x -0.5, coords.y/resolution.y -0.5) * 3.141592653 * 100;
    vec2 polar = vec2(
            atan(coords.x, coords.y),
            sqrt(coords.x*coords.x + coords.y*coords.y)
        );
    fragColor = vec4(pattern01(polar + cos(t*t*0.1), 6.0 + sin(t*t*0.2)*0.05)) + pattern02(coords + sin(t+polar.r)*5.0, 8+0.02*sin(t)) + 1;
}
